...
uses
  ...
  DBDateTimePicker, DCPrc6, DCPrc4, DCPmd5, DCPripemd160, DCPsha1, Forms,
  Controls, Graphics, Dialogs, ComCtrls, StdCtrls, DBCtrls, Buttons, DBExtCtrls,
  ExtCtrls, XMLPropStorage, Menus, IniPropStorage;
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure RunOnece_one(Sender: TObject);
    procedure ifSizeOFLog(Sender: TObject);
...
{$R *.lfm}
uses medikamente_unit, monat_jahr_unit, my_mail_unit, smtpsend,
  ssl_openssl, Mimepart, Mimemess, INIFiles;
...
begin
  ...
  SQLQuery2.SQL.Text :=
    'CREATE TABLE IF NOT EXISTS tblJahr ( ID AUTO_INCREMENT Primary KEY, Jahr VARCHAR(6))';
  SQLQuery2.ExecSQL;
  SQLTransaction1.commit;
  ...
  SQLQuery3.SQL.Text :=
    'CREATE TABLE IF NOT EXISTS tblMonth (ID AUTO_INCREMENT Primary KEY, Month VARCHAR(25))';
  SQLQuery3.ExecSQL;
  SQLTransaction1.commit;
  ...
  RunOnece_one(self);
  ShowMessage('RunOnce wird ausgeführt');
  ...
  begin
    ifSizeOFLog(self);
    ShowMessage('Run_Logfilesize wird ausgeführt');
  end;
end;
...
begin
  ...
  fromMail := INI.ReadString('INIDB', 'fromMail', '');
  toMail := INI.ReadString('INIDB', 'toMail', '');
...
//if InputQuery('Passphrase','Enter passphrase',KeyStr) then  // get the passphrase
  ...
  begin
    ...
    try
      ...
      m.header.from := fromMail;
      m.header.tolist.add(toMail);
